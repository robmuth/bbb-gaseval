...
contract VotingWithERC20Token is IERC20 {
  ...
  */
  ...
  function vote(uint _option, uint256 _weight) public {
      require(_option <= maxOption);
      require(_weight > 0);
      assert(votes[_option] < 2**256 - 1);

      require(balanceOf(msg.sender) >= _weight, "ASJDKASJDKJASDKLASD");
      _burn(msg.sender, _weight);

      votes[_option] += _weight;
  }
}
