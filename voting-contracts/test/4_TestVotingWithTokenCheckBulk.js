const VotingWithTokenCheckBulk = artifacts.require("VotingWithTokenCheckBulk");

const MAX_VOTE = 5; // must be >= 2!

contract("VotingWithTokenCheckBulk", (accounts) => {
	var contract;

	before(() => { 
		return VotingWithTokenCheckBulk.new(
				MAX_VOTE, // max. vote
			)
	...
	it("should not vote 10 times", async () => {
		let option = MAX_VOTE - 1;

		let votes = await contract.votes.call(option);

		let privateTokens = [];
		for(let i = 0; i < 10; i++) {
			let privateTokenStr = "PR1V4T3" + i;
			let privateToken = await contract.generatePrivateToken.call(privateTokenStr);
			let publicToken = await contract.generatePublicToken.call(privateToken);
			await contract.addToken(publicToken, { from: accounts[0]});
		...
		for(let i = 0; i < 10; i++) {
			try {
				await contract.vote(option, privateTokens[i]);
			} catch(e) {
				assert.fail("Vote should not fail.");
			}
		}

		assert.equal(await contract.votes.call(option), parseInt(votes) + 10, "Votes for " + option + " should have increased by 10.");
	});
});